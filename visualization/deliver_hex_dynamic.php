...
if($_GET["zoom"]<4){
	$sql = 'SELECT COUNT(*), zh'.$z0i.' FROM `'.$db_table.'` WHERE `validconversion` = 1 AND ('.$yearcheck.') GROUP BY zh'.$z0i;	
}else{
	$sql = 'SELECT COUNT(*), zh'.$z0i.' FROM `'.$db_table.'` WHERE `validconversion` = 1 AND ('.$yearcheck.') AND x0 > '.$_GET['min_x'].' AND x0 < '.$_GET['max_x'].' AND y0 > '.$_GET['min_y'].' AND y0 < '.$_GET['max_y'].' GROUP BY zh'.$z0i;	
}
