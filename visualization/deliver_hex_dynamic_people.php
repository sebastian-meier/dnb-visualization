...
$start_y = floor(($_GET["start"]/10))*10;
$end_y = ceil(($_GET["end"]/10))*10;
...
$years = "";
for($y = $start_y; $y<=$end_y; $y+=10){
	if($years != ""){$years .= " OR ";}
 	$years .= ' `key` = "maxh_'.$y.'_person" ';
}

$total_max = 0;
$sql = 'SELECT value FROM `'.$db_max_table.'` WHERE `zoom` = '.$z0i.' AND ('.$years.')';
...
$div = ($end_y - $start_y)/10;

$yearcheck = "";
$yearselect = "";
for($y = $start_y; $y<=$end_y; $y+=10){
	if($yearcheck != ""){$yearcheck .= " OR ";}
	$yearcheck .= " hits_".$y." > 0 ";

	if($yearselect != ""){$yearselect .= " + ";}
	$yearselect .= " sum(hits_".$y.") ";
}

if($_GET["zoom"]<4){
	$sql = 'SELECT (('.$yearselect.')/'.$div.'), zh'.$z0i.' FROM `'.$db_table.'` WHERE `validconversion` = 1 AND ('.$yearcheck.') GROUP BY zh'.$z0i;	
}else{
	$sql = 'SELECT (('.$yearselect.')/'.$div.'), zh'.$z0i.' FROM `'.$db_table.'` WHERE `validconversion` = 1 AND ('.$yearcheck.') AND x0 > '.$_GET['min_x'].' AND x0 < '.$_GET['max_x'].' AND y0 > '.$_GET['min_y'].' AND y0 < '.$_GET['max_y'].' GROUP BY zh'.$z0i;	
}
